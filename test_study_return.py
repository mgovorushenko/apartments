...
near(edge(finish,0,1),edge(wall,0,1)+.015) # miter extends to the adjacent finish face
near(edge(finish,2,1),edge(wall,2,-1))
near(edge(finish,1,-1),model.FINISH_SETTINGS['dry_floor'])
near(edge(finish,1,1),2.8-model.FINISH_SETTINGS['ceiling_drop']+.003)
...
near(edge(skirt,0,-1),edge(node('DoorCasing_Study_A_Right_'),0,1))
near(edge(skirt,0,1),edge(node('Skirting_FinishBeige_HallStudyReturn_'),0,1))
